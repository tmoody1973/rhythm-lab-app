...
interface StoryblokAboutResponse {
  success: boolean
  story?: any
  message?: string
  error?: string
...
/**
 * GET /api/storyblok/about
 * Fetch About page content from Storyblok
 */
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.log('Fetching About page from Storyblok...')

    // Try different possible slugs for the About page story
    const possibleSlugs = [
      'about',
      'about-page',
      'about-us',
    ...
    for (const slug of possibleSlugs) {
      try {
        ...
        if (response.ok) {
          ...
          foundSlug = slug
          console.log(`Found About page with slug: ${slug}`)
          break
        }
      } catch (error) {
    ...
    if (!foundSlug || !response?.ok) {
      console.error('About page not found with any of the expected slugs:', possibleSlugs)
      return NextResponse.json({
        success: false,
        error: 'About page not found in Storyblok'
      }, { status: 404 })
    }
    ...
    return NextResponse.json({
      success: true,
      story: data.story,
      message: `About page loaded successfully from slug: ${foundSlug}`
    })
