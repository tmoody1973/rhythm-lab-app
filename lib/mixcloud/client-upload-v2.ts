...
export async function uploadToMixcloudDirect(
  ...
  try {
    ...
    // Set up progress tracking (simulated)
    ...
    if (progressInterval) {
      ...
      if (onProgress) onProgress(90) // Upload complete, processing response
    }
...
/**
 * Process the upload response from Mixcloud
 */
async function processResponse(response: Response): Promise<MixcloudUploadResult> {
